...
// Type guard para distinguir entre GroupInvitation y GroupJoinRequest
function isGroupInvitation(invitation: GroupInvitation | GroupJoinRequest): invitation is GroupInvitation {
  return 'id_invitation' in invitation && 'inviter' in invitation;
}
...
export const GroupInvitationCard: React.FC<GroupInvitationCardProps> = ({
  ...
  const handleAccept = async () => {
    setIsLoading(true);
    try {
      if (isGroupInvitation(invitation)) {
        // Es invitación
        if (!invitation.group?.id_group || !invitation.id_invitation) return;
        ...
        if (!invitation.group?.id_group || !invitation.id_request) return;
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.leftSection}>
        {isGroupInvitation(invitation) && invitation.inviter?.picture ? (
          <Image
      ...
      <View style={styles.content}>
        <Text style={styles.name}>
          {isGroupInvitation(invitation) 
            ? invitation.inviter?.full_name 
            : invitation.requester?.full_name || 'Usuario'}
        </Text>
