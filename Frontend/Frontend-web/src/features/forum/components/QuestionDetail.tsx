...
export const QuestionDetail: React.FC<QuestionDetailProps> = ({
  ...
  const handleVoteAnswer = async (answerId: number) => {
    setAnswers((prev) => sortAnswers(prev.map((a) => a.id === answerId ? { ...a, voteCount: a.voteCount + 1 } : a)));
    try {
  ...
  const handleAcceptAnswer = async (answerId: number) => {
    try {
      ...
      setAnswers((prev) => sortAnswers(prev.map((a) => ({ ...a, isAccepted: a.id === answerId }))));
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.questionCard}>
        ...
        <span className={styles.meta}>Por · {question.authorName}</span>
      </div>
            ...
            {answers.map((answer) => (
              <li key={answer.id} className={`${styles.answerItem} ${answer.isAccepted ? styles.answerAccepted : ''}`}>
                ...
                <div className={styles.answerContent}>
                  <button className={styles.voteBtn} onClick={() => handleVoteAnswer(answer.id)}>
                    <ChevronUp size={15} />
                    <span>{answer.voteCount}</span>
                  </button>
                  <div className={styles.answerBody}>
                    <p>{answer.body}</p>
                    <span className={styles.meta}>Por · {answer.authorName}</span>
                  </div>
                  {isTeacher && !answer.isAccepted && question.status === 'OPEN' && (
                    <button className={styles.acceptBtn} onClick={() => handleAcceptAnswer(answer.id)}>
                      <CheckCircle size={14} />
                      Aceptar
                    </button>
                  )}
      ...
      <form className={styles.replyBox} onSubmit={handleSubmitAnswer}>
        <textarea
          ...
          rows={3}
        />
        <button type="submit" className={styles.replyBtn} disabled={!answerBody.trim() || submitting}>
          <Send size={15} />
